...
@pytest.fixture
def s3_obj_factory(mocker) -> Callable[[str], MagicMock]:
...
def test_get_count_from_object__one_count(s3_obj_factory):
    one_count_obj = s3_obj_factory('count: 10')
    assert get_count_from_object(one_count_obj) == ['10']


def test_get_count_from_object__one_count_capital_spaces(s3_obj_factory):
    one_count_obj = s3_obj_factory('COUNT  :  15')
    assert get_count_from_object(one_count_obj) == ['15']


def test_get_count_from_object__one_count_embedded_in_text(s3_obj_factory):
    one_count_obj = s3_obj_factory('Test text COUNT  :  15 blah')
    assert get_count_from_object(one_count_obj) == ['15']


def test_get_count_from_object__no_count(s3_obj_factory):
    one_count_obj = s3_obj_factory('Test text  :  15 blah')
    assert get_count_from_object(one_count_obj) == []


def test_get_count_from_object__empty_string(s3_obj_factory):
    one_count_obj = s3_obj_factory('')
    assert get_count_from_object(one_count_obj) == []


def test_get_count_from_object__two_counts_raises(s3_obj_factory):
    two_count_obj = s3_obj_factory('Yappity yap count:  10 more waffle COUNT: 12 piffle paffle')
